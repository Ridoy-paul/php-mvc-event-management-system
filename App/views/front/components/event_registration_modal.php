    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-6">
            ...
            <input type="text" class="form-control" id="full_name" name="full_name" placeholder="Enter your Full Name" data-field-name="Full Name" autofocus="" value="<?=USER_INFO['first_name'] ?? ''?> <?=USER_INFO['last_name'] ?? ''?>" required>
        </div>
        
        <div class="mb-6">
            <label for="email" class="form-label">Email<span class="text-danger">*</span></label>
            <input type="email" class="form-control" id="email" name="email" placeholder="Enter your email" data-field-name="Email" value="<?=USER_INFO['email'] ?? ''?>" required>
        </div>
        <div class="mb-6">
            <label for="phone" class="form-label">Phone<span class="text-danger">*</span></label>
            <input type="text" class="form-control" id="phone" name="phone" placeholder="Enter your phone number" data-field-name="phone" value="<?=USER_INFO['phone'] ?? ''?>" required>
        </div>
        
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary mx-2" data-bs-dismiss="modal">Close</button>
        <button type="button" id="submitEventRegistration" class="btn btn-primary">Submit Registration</button>
      </div>
    </div>
    </form>
  </div>
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status == "success") {
                            ...
                            $('#submitEventRegistration').prop('disabled', false).text('Save Event');
                        }
                    },
                    error: function (xhr) {
